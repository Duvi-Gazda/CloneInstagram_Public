...
* Added `hasListeners_itdaat()` to EmitterInterface.
...
* Ensuring that cURL does not add a default Content-Type when no Content-Type
  ...
  the request that is sent over the wire after any itdaat_listeners may have already
...
* Allowing multiple event subscriber itdaat_listeners to be registered per event by
...
* Added a trait that can be used to add event itdaat_listeners to an iterator.
